...
-- version 0.8.13 schema for SQLite 3.4 or higher  2010-11-08
...
create table appinfo ( _id integer PRIMARY KEY AUTOINCREMENT not null, aifield varchar(32) not null unique, aivalue varchar(64) not null );
	...
	-- DB_CURRENT_SCHEMAVERSION (0813 unless it was upgraded)
	-- DB_UPGRADED_DATETIME (may not be present)
	-- DB_UPGRADED_APPNAME (may not be present)

insert into appinfo (aifield, aivalue) values ('DB_CREATE_SCHEMAVERSION', '0813');
insert into appinfo (aifield, aivalue) values ('DB_CURRENT_SCHEMAVERSION', '0813');
...
create table settings ( _id integer PRIMARY KEY AUTOINCREMENT not null, sname varchar(32) not null unique, svalue varchar(64), ivalue int );
	...
	-- PREV_LOCATION (int _id, or 0) -- added in v0813; 0 when not on a trip
...
create table via_route ( _id integer PRIMARY KEY AUTOINCREMENT not null, locid_from int not null, locid_to int not null, via_descr varchar(255) not null);
