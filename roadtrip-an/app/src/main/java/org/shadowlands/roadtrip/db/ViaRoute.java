...
public class ViaRoute extends RDBRecord
{
	...
	private static final String DESCFIELD_SORT = "via_descr COLLATE NOCASE";  // syntax is sqlite-specific
	...
	/**
	 * Retrieve all ViaRoutes between 2 {@link Location}s.
	 * @param db  db connection
	 * @param locFrom  {@link Location} to start from
	 * @param locTo  {@code Location} to go to
	...
	/**
	 * Retrieve all ViaRoutes from a given {@link Location} ID to another location.
	 * This method is not bidirectional; see {@link #getAll(RDBAdapter, int, int, boolean)}.
	 * @param db  db connection
	 * @param locID_from  {@link Location} to start from, or -1 for all ViaRoutes in the database
	 * @param locID_to  {@code Location} to go to
	 * @return ViaRoutes between these locations, ordered by description, or null if none
	 * @throws IllegalStateException if db not open
	 */
		...
		throws IllegalStateException
	{
		...
		if (locID_from != -1)
		{
			final String[] locIDsFromTo = new String[]
				{ Integer.toString(locID_from), Integer.toString(locID_to) };
			sv = db.getRows
	...
	private static ViaRoute[] toArray(final RDBAdapter db, final Vector<String[]> sv)
	{
		...
		catch (RDBKeyNotFoundException e) { }  // not thrown, but catch required
	 ...
	 * @since 0.9.40
	 */
	public static ViaRoute getByLocsAndDescr
		(RDBAdapter db, final int locID_from, final int locID_to, final String descr)
		throws IllegalStateException
	{
		try {
			...
			Vector<String[]> vias = db.getRows
				(TABNAME, "locid_from=? and locid_to=? and via_descr COLLATE NOCASE = ?",
				 locIDsAndDescr, FIELDS_AND_ID, "_id", 0);
			if (vias != null)
				return new ViaRoute(db, vias.firstElement());  // <--- found ---
		}
